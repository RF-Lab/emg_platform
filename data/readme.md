...
Если обратиться, например, к первому сигналу df['Сигнал'][0], то будет возвращена строка, в которую обернут массив отсчетов. Для того чтобы получить сам массив отсчетов, необходимо написать eval(df['Сигнал'][0]).
