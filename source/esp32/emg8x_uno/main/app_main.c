...
//
// Single channel version firmware for 8 channel EMG8x board
//
            ...
            while(1)
            {
                ...
                if (drdy_thread_context.tail!=drdy_thread_context.head)
                {
                    ...
                    if( write(cs , drdy_thread_context.adcDataQue[drdy_thread_context.tail],
                        ((CONFIG_EMG8X_TRANSPORT_BLOCK_HEADER_SIZE)/sizeof(int32_t)+(CONFIG_EMG8X_SAMPLES_PER_TRANSPORT_BLOCK))*sizeof(int32_t)) < 0)
                    {
...
IRAM_ATTR void spi_data_pump_task( void* pvParameter )
{
    ...
    while(1)
    {
        ...
        for(int chNum=0;chNum<1;chNum++)
        {
            ...
            int byteOffsetCh            = (chNum+2)*3 ; // offset in raw data to first byte of 24 twos complement field
